...
for freq in antennas:
    ...
    for i,(a,b) in enumerate(pairs):
        antinodes[freq][a] = True
        antinode_a = add(a, distances_1_2[i])
        while in_bound(antinode_a):
            antinodes[freq][antinode_a] = True
            antinode_a = add(antinode_a, distances_1_2[i])

        antinodes[freq][b] = True
        antinode_b = add(b, distances_2_1[i])
        while in_bound(antinode_b):
            antinodes[freq][antinode_b] = True
            antinode_b = add(antinode_b, distances_2_1[i])
